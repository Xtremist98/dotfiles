...
BarWidget {
  ...
  // Tint every tray icon to the bar foreground (color04) by default so the tray
  // matches the rest of the bar. Set "forceColorizeAll": false on the tray entry
  // to go back to only symbolic icons + the forceColorize list.
  readonly property bool forceColorizeAll: settings.forceColorizeAll !== false
  ...
  // entry) to restore the pinned/drawer split.
  readonly property bool expandByDefault: settings.expandByDefault === true
  ...
  function classifyItem(item) {
    ...
    if (root.expandByDefault) return "pinned"
  ...
  Component {
    ...
    Item {
      ...
      readonly property int iconsExtent: root.allItems.length * root.traySlot
        + (root.allItems.length - 1) * root.trayItemGap
      readonly property int pillInnerPad: Style.space(7)
      readonly property int pillHeight: root.expanded && hasItems
        ? iconsExtent + pillInnerPad * 2
        : 0

      implicitWidth: root.barSize
      implicitHeight: root.bar && root.bar.vertical
        ? (root.expanded && hasItems ? pillHeight : (hasItems ? root.chevronSize : 0))
        : 0
      ...
      HoverHandler {
        onHoveredChanged: verticalTrayRoot.hovered = hovered
      }
      ...
      Rectangle {
        ...
        Column {
          ...
          anchors.topMargin: verticalTrayRoot.pillInnerPad
          spacing: root.trayItemGap
  ...
  component TrayIcon: Item {
    ...
    readonly property bool symbolic: root.iconIsSymbolic(icon) || root.forceColorizeItem(icon) || root.forceColorizeAll
